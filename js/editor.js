...
const ResumeEditor = (function () {
    ...
    function getFocusableElements() {
        ...
        return Array.from(document.getElementById('editorOverlay').querySelectorAll(selector)).filter(
    ...
    function addSkillChip(skill) {
        const $chip = $(
            '<span class="skill-chip"><span class="skill-chip__label"></span><button type="button" aria-label="Remove skill">&times;</button></span>'
        );
    ...
    function isStateEmpty(s) {
        ...
        const hasLists = ['experience', 'projects', 'education', 'certificates'].some((key) =>
            (s[key] || []).some(entryHasAnyContent)
        );
    ...
    function updateEntryFieldIndicators(type, fields) {
        ...
        $(ENTRY_CONFIG[type].listSelector)
            .children('.entry-card')
            .each(function () {
                const $card = $(this);
    ...
    function hasCompleteEntry(type, fields) {
        return $(ENTRY_CONFIG[type].listSelector)
            .children('.entry-card')
            .toArray()
            .some(function (card) {
    ...
    function accordionSectionByTarget(target) {
        return ACCORDION_SECTIONS.find(function (s) {
    ...
    function showSectionConfirm(currentLabel, target) {
        ...
        $('#sectionConfirmMessage').text(
            'You still have required fields left in ' +
                currentLabel +
                '. You can continue anyway, or go back and finish it first.'
        );
    ...
    function exportDocumentTitle() {
        const name = [state.personal.firstName, state.personal.lastName]
            .map((s) => (s || '').trim())
            .filter(Boolean)
            .join(' ');
    ...
    function bindEvents() {
        ...
        $(document).on(
            'input change',
            '.editor-form-panel input:not(#skillInput):not(#photoInput):not(#photoVisibleInput):not([name="photoPosition"]):not(.section-order-row input), .editor-form-panel textarea',
            syncFromForm
        );
        ...
        $(document).on('click', '.entry-card__arrow', function () {
            ...
            if (direction === 'up') $card.insertBefore($sibling);
            else $card.insertAfter($sibling);
        ...
        $(document).on('drop', '.entry-card[draggable="true"]', function (e) {
            ...
            if (fromIndex < toIndex) $(dragEntryCard).insertAfter(this);
            else $(dragEntryCard).insertBefore(this);
            updateEntryArrowStates($list);
            syncFromForm();
        });
        ...
        $(document).on('drop', '.skill-chip[draggable="true"]', function (e) {
            ...
            if (fromIndex === -1 || toIndex === -1) return;
            if (fromIndex < toIndex) $(dragSkillChip).insertAfter(this);
            else $(dragSkillChip).insertBefore(this);
            syncFromForm();
        });
        ...
        $('#exportPdfBtn').on('click', function () {
            ...
            if (score < MIN_EXPORT_SCORE) {
                ResumeToast.show(
                    'Your resume is only ' +
                        score +
                        '% complete. Fill in more details to reach ' +
                        MIN_EXPORT_SCORE +
                        "%+ before exporting — check the Resume Score badge to see exactly what's missing.",
                    'warning'
                );
                return;
            }
        ...
        $(document).on('keydown', function (e) {
            ...
            if (e.key === 'Escape') {
                ...
                if (
                    $(
                        '#scorePanel, #feedbackModalBackdrop, #sectionConfirmBackdrop, #assistantPanel, .tour-tooltip'
                    ).is(':visible')
                )
                    return;
